...
void lcd_set_mode(unsigned int mode)
{
	...
	fprintf(fp, "%c%s", ESC, CLEAR_LCD_STRING);
	fprintf(fp, "Mode: %d\n", mode);
}
...
alt_u32 tlc_timer_isr(void* context)
{
	volatile int* timeOut_num = (volatile int*)context;

	(*timeOut_num)++;

	int index = (int)&timeOut_num;

	printf("timeout: %i", timeout[index]);
...
int main(void)
{
	...
	void* tlc_timer_context = (void*) tlc_timer_event;
	alt_alarm_start(&tlc_timer, 0, tlc_timer_isr, tlc_timer_context);
